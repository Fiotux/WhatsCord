...
const fs = require('fs');
const { Client: DiscordClient, AttachmentBuilder, EmbedBuilder, GatewayIntentBits, WebhookClient, ChannelType } = require('discord.js');
  ...
  if (msg.author.id != config.discordaccountid) return;
    ...
    let data;
        ...
        if(msg.reference != null) {
          const refmsg = await Messages.findOne({ where: { did: `${msg.reference.messageId}` } });
          const waref = await client.getMessageById(`${refmsg.wid}`);
          if (msg.attachments.first()) {
            if(msg.content == "") {
              const wamsg = await waref.reply(MessageMedia.fromUrl(msg.attachments.first().url))
            ...
            const media = await MessageMedia.fromUrl(msg.attachments.first().url).catch(
              waref.reply(MessageMedia.fromUrl(msg.attachments.first().url))
            );
            if (msg.content) {
              if (msg.attachments.first().url.includes('SPOILER_')) {
                  const wamsg = await waref.reply(msg.content, null, { sendAudioAsVoice: config.sendaudioasvoice, media: media, isViewOnce: true });
                  await Messages.create({
                  wid: `${wamsg.id._serialized}`,
                  did: `${msg.id}`,
                  channel: `${msg.channel.id}`
                ...
                const wamsg = await waref.reply(msg.content, null, { sendAudioAsVoice: config.sendaudioasvoice, media: media });
                await Messages.create({
                  wid: `${wamsg.id._serialized}`,
                  did: `${msg.id}`,
                  channel: `${msg.channel.id}`
              ...
              if (msg.attachments.first().url.includes('SPOILER_')) {
                const wamsg = await waref.reply(media, null, { sendAudioAsVoice: config.sendaudioasvoice, isViewOnce: true });
                await Messages.create({
                  wid: `${wamsg.id._serialized}`,
                  did: `${msg.id}`,
                  channel: `${msg.channel.id}`
                ...
                const wamsg = await waref.reply(media, null, { sendAudioAsVoice: config.sendaudioasvoice });
                await Messages.create({
                  wid: `${wamsg.id._serialized}`,
                  did: `${msg.id}`,
                  channel: `${msg.channel.id}`
            ...
            const wamsg = await waref.reply(media, null, { sendMediaAsSticker: true });
            await Messages.create({
              wid: `${wamsg.id._serialized}`,
              did: `${msg.id}`,
              channel: `${msg.channel.id}`
            ...
            const wamsg = await waref.reply(msg.content);
            await Messages.create({
              wid: `${wamsg.id._serialized}`,
              did: `${msg.id}`,
              channel: `${msg.channel.id}`
...
client.on('message_edit', (msg, newBody) => {
  (async () => {
    ...
    const data = await Messages.findOne({ where: { wid: `${msg.id._serialized}` } });
    if (!data) return;
    const channel = await bot.channels.fetch(`${data.channel}`);
    embed = new EmbedBuilder()
	          .setColor(5763719)
            .setDescription(`${newBody}`)
    const fetchedMessage = await channel.messages.fetch(`${data.did}`);
    if(msg.getChat().isGroup) {
